...
export function setEntryDefaults(history,selected){const latest=exerciseHistory(history,selected?.exercise,selected?.group)[0];if(selected?.group==='Stretches')return {timePerSide:Number.isInteger(latest?.timePerSide)?latest.timePerSide:30};return latest&&latest.timePerSide===undefined?{repetitions:latest.repetitions,weight:latest.weight,weightUnit:latest.weightUnit||'kg',speed:latest.speed}:{repetitions:10,weight:0,weightUnit:'kg',speed:'normal'}}
