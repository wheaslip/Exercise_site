...
const stretchState=()=>{const state=freshState(()=>0);state.config.groups=[{name:'Stretches',weight:1,display:{type:'body',regions:['upper-legs']},exercises:['hamstring stretch']}];state.selected={group:'Stretches',exercise:'hamstring stretch'};return state};
...
test('Stretches completion records only time per side',()=>{const s=stretchState(),now=new Date(2026,0,2,12),done=complete(s,{...s.selected,timePerSide:'45',repetitions:99,weight:50,weightUnit:'lb',speed:'plyometric'},now,()=>0,()=> 'stretch');assert.deepEqual(done.history['2026-01-02'].events[0],{id:'stretch',timestamp:now.toISOString(),date:'2026-01-02',group:'Stretches',exercise:'hamstring stretch',timePerSide:45})});
test('Stretches completion requires a positive whole number of seconds per side',()=>{const s=stretchState();for(const timePerSide of [0,-1,1.5,''])assert.throws(()=>complete(s,{...s.selected,timePerSide}),/Time per side/)});
test('completion stores a trimmed optional note and omits blank notes',()=>{const s=freshState(()=>0),noted=complete(s,set(s.selected,{note:'  Left knee felt good  '}),new Date(2026,0,2,12),()=>0,()=> 'noted'),blank=complete(s,set(s.selected,{note:'   '}),new Date(2026,0,2,13),()=>0,()=> 'blank');assert.equal(noted.history['2026-01-02'].events[0].note,'Left knee felt good');assert.equal('note' in blank.history['2026-01-02'].events[0],false)});
...
test('stretch entry defaults use the latest time per side and otherwise use 30 seconds',()=>{const stretch={...events(1)[0],group:'Stretches',exercise:'hamstring stretch',repetitions:undefined,weight:undefined,weightUnit:undefined,speed:undefined,timePerSide:45};assert.deepEqual(setEntryDefaults({'2026-01-01':{events:[stretch]}},{group:'Stretches',exercise:'hamstring stretch'}),{timePerSide:45});assert.deepEqual(setEntryDefaults({}, {group:'Stretches',exercise:'new'}),{timePerSide:30})});
...
test('set workload uses stretch time per side',()=>assert.equal(setWorkload({group:'Stretches',timePerSide:45}),45));
